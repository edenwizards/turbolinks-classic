...
const enableProgressBar = function(enable) {
  ...
  if (enable) {
    ...
    if (progressBar) { progressBar.uninstall(); }
...
const popCookie = function(name) {
  ...
  const value = (match && match[1].toUpperCase()) || '';
...
var processResponse = function() {
  ...
  const extractTrackAssets = doc => (() => {
    ...
    for (var node of Array.from(doc.querySelector('head').childNodes)) {
      if (node.getAttribute && (node.getAttribute('data-turbolinks-track') != null)) {
...
var extractTitleAndBody = function(doc) {
  ...
  return [ title && title.textContent, removeNoscriptTags(doc.querySelector('body')), CSRFToken.get(doc).token, 'runScripts' ];
};
...
var CSRFToken = {
  get(doc) {
    ...
    return {
      ...
      token:  (tag && tag.getAttribute) ? tag.getAttribute('content') : undefined
    };
  },
